...
fn parse_args() -> (
    ...
    while let Some(a) = it.next() {
        match a.as_str() {
            ...
            "--rows" => {
                rows_opt = Some(it.next().unwrap().parse().unwrap());
            }
            "--cols" => {
                cols_opt = Some(it.next().unwrap().parse().unwrap());
            }
            ...
            "--json" => json = true,
            "--graph-file" => {
                let v = it.next().expect("--graph-file value");
                graph_file = Some(PathBuf::from(v));
            }
            "--sources-file" => {
                let v = it.next().expect("--sources-file value");
                sources_file = Some(PathBuf::from(v));
            }
            _ => {}
        }
    }
    if rows_opt.is_some() || cols_opt.is_some() {
        grid_rc = Some((rows_opt.unwrap_or(1), cols_opt.unwrap_or(1)));
    }
    (
        graph,
        n,
...
fn make_ba(n: usize, m0: usize, m: usize, maxw: u32, seed: u64) -> Graph {
    ...
    for u in start..n {
        for _ in 0..m {
            // pick endpoints proportional to degree
...
fn main() {
    ...
    let (g, gname): (Graph, &'static str) = if let Some(path) = graph_file.as_ref() {
        (
            read_graph_from_file(path).expect("failed to read graph file"),
            match gtype {
                GraphType::Grid => "grid",
                GraphType::ER => "er",
                GraphType::BA => "ba",
    ...
    for t in 0..trials {
        ...
        let res = if threads > 1 {
            bmssp_sharded(&g, &sources, b, threads)
        } else {
            bounded_multi_source_shortest_paths(&g, &sources, b)
        };
    ...
    if let Some(b) = best {
        eprintln!("best ns={} popped={} B'={}", b.time_ns, b.popped, b.b_prime);
    }
}
